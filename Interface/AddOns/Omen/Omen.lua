...
function Omen:CreateFrames()
	...
	self.Anchor:SetResizeBounds(90, db.Background.EdgeSize * 2)
	...
	self.Title:SetResizeBounds(90, db.Background.EdgeSize * 2)
...
function Omen:SetAnchors(useDB)
	...
	self.Anchor:SetUserPlaced(false)
...
function Omen:UpdateBackdrop()
	...
	self.Anchor:SetResizeBounds(90, h)
	self.Title:SetResizeBounds(90, h)
	...
	if not self.FlashFrame then
		...
		flasher:SetScript("OnUpdate", function(self, elapsed)
			...
			if elapsed < 2.6 then
				...
				if alpha < 0.15 then
					self:SetAlpha(math.abs(alpha / 0.15))
				elseif alpha < 0.9 then
					self:SetAlpha(math.abs(1 - (alpha - 0.15) / 0.6))
				else
					self:SetAlpha(0)
				end
			else
